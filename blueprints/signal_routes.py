...
def calculate_signal_score_csv():
    ...
    # Daily quota via the standard rate limiter (anon: 3/day, tiers per
    # tier limits in usage_log).
    allowed, info = check_rate_limit("signal_score_csv")
    if not allowed:
        return jsonify({
            "ok": False,
            "error": "Daily upload limit reached. Sign up or upgrade for higher limits.",
            "rate_limit": info,
        }), 429
    ...
    # Count this upload against the daily rate limit.
    try:
        log_usage("signal_score_csv")
    except Exception:
        logger.exception("Non-fatal: failed to log CSV usage")
